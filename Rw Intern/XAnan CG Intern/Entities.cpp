...
{
    bool* SpottedStatus = reinterpret_cast<bool*>(addr + Offsets::bSpottedByMask);
    *SpottedStatus = 1;
}
...
void CEntities::Update() {
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        if (Globals::ESP::rader) {

            if (!Globals::Misc::TeamCheck) {
                const auto EntityTeam = *reinterpret_cast<int*>(EntityData.Controller + Offsets::Team);
                if (EntityTeam == LocalTeam)
                    continue;
                RadarHack(EntityData.Pawn);
            }
            else {
                RadarHack(EntityData.Pawn);
            }
