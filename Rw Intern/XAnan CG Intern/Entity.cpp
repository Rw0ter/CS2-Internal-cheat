...
void Set::GlowHack(intptr_t addr)
{
	...
	*GlowColor = ConvertImColorToInt(ImGlowColor);
	*glow = 1;
}
...
{
	// 将 ImColor 的每个分量缩放到 0 到 255 的范围，并进行取整
	unsigned char r = static_cast<unsigned char>(color.Value.x * 255.0f);
	unsigned char g = static_cast<unsigned char>(color.Value.y * 255.0f);
	unsigned char b = static_cast<unsigned char>(color.Value.z * 255.0f);
	unsigned char a = static_cast<unsigned char>(color.Value.w * 255.0f);

	// 使用位操作组合为一个 int，通常使用 ARGB 格式
